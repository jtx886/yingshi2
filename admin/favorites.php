...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        db()->delete('favorites', 'id = ?', array($id));
        json_success(null, '收藏已删除');
    }
    if ($action === 'clear_user') {
        $uid = intval($_POST['user_id']);
        db()->delete('favorites', 'user_id = ?', array($uid));
        json_success(null, '已清空该用户的收藏');
    }
    if ($action === 'clear_all') {
        db()->raw("TRUNCATE TABLE favorites");
        json_success(null, '已清空全部收藏');
    }
    json_error('未知操作');
}
...
if ($userId > 0) { $where .= ' AND f.user_id = ' . $userId; }
if ($search) { $where .= ' AND (f.movie_name LIKE ? OR u.username LIKE ?)'; $params[]="%{$search}%"; $params[]="%{$search}%"; }

try {
    $total = db()->fetchOne("SELECT COUNT(*) c FROM favorites f LEFT JOIN users u ON f.user_id=u.id WHERE {$where}", $params);
    $totalCnt = intval($total['c']); $totalPages = ceil($totalCnt/$perPage);
    $avgRating = db()->fetchOne("SELECT COALESCE(AVG(f.rating),0) r FROM favorites f LEFT JOIN users u ON f.user_id=u.id WHERE {$where}", $params);
    $avgR = floatval($avgRating['r']);
    $list = db()->fetchAll(
        "SELECT f.*, u.username, u.email FROM favorites f LEFT JOIN users u ON f.user_id=u.id WHERE {$where} ORDER BY f.created_at DESC LIMIT {$offset}, {$perPage}",
        $params
    );
} catch(Exception $e){ $list=array(); $totalCnt=0; $totalPages=1; $avgR=0; }
?>
...
<div class="stats-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:20px;">
    ...
    <div class="stat-card views">
        ...
        <div class="stat-value"><?php echo $avgR>0?number_format($avgR,1):'-';?></div>
    </div>
...
<div class="admin-table-wrapper">
    <div class="admin-table-header">
        <div class="admin-table-title">❤️ 用户收藏</div>
        <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;">
            ...
            <a href="<?php echo BASE_URL;?>/admin/favorites.php" class="btn btn-outline btn-sm">重置</a>
            <?php if($userId>0):?>
        ...
        <tr>
            <td>
                <div style="width:52px;height:72px;border-radius:6px;overflow:hidden;background:var(--bg-dark);position:relative;">
                    ...
                    <img src="<?php echo e($f['poster']);?>" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentElement.innerHTML='<div style=\\'display:flex;align-items:center;justify-content:center;height:100%;font-size:22px;color:var(--text-muted);\\'>🎬</div>'">
                    <?php else:?>
            ...
            <td>
                <div style="font-weight:500;"><?php echo e($f['movie_name']);?></div>
                <div style="font-size:11px;color:var(--text-muted);">类型：<?php echo e($f['movie_type']);?> · TMDB_ID: <?php echo intval($f['tmdb_id']);?></div>
            </td>
            <td>
                <div><?php echo $f['username']?e($f['username']):'<span style="color:var(--text-muted);">—</span>';?></div>
                <div style="font-size:11px;color:var(--text-muted);"><?php echo $f['email']?e($f['email']):'';?></div>
            </td>
            <td>
                <?php if($f['rating']>0):?>
                <div style="display:flex;align-items:center;gap:6px;">
                    <span style="color:#f59e0b;font-weight:700;"><?php echo number_format($f['rating'],1);?></span>
                    <span style="color:#f59e0b;">★</span>
                </div>
                <?php else:?>
                <span style="color:var(--text-muted);font-size:12px;">未评分</span>
                <?php endif;?>
            </td>
            <td style="color:var(--text-muted);font-size:12px;"><?php echo format_time($f['created_at'],true);?></td>
            <td>
                <div class="action-buttons">
                    <?php if(!empty($f['tmdb_id'])):?>
                    <a class="action-btn edit" target="_blank" href="<?php echo BASE_URL;?>/detail.php?id=<?php echo intval($f['tmdb_id']);?>&type=<?php echo e($f['movie_type']);?>">👁</a>
                    <?php endif;?>
                    <button class="action-btn delete" onclick="delRecord(<?php echo intval($f['id']);?>)">🗑</button>
                </div>
            </td>
        </tr>
        <?php endforeach;?>
        <?php if(empty($list)):?>
        <tr><td colspan="6" style="text-align:center;padding:60px;color:var(--text-muted);">📭 暂无收藏记录</td></tr>
        <?php endif;?>
        </tbody>
    </table>
...
function clearUser(uid){
    if(!confirm('确定清空该用户的全部收藏？'))return;
...
function clearAll(){
    if(!confirm('⚠️ 确定清空全站所有用户的收藏？此操作无法撤销！'))return;
    if(!confirm('⚠️ 再次确认！'))return;
