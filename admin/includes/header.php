...
:root {
    ...
    --primary-gradient: linear-gradient(135deg, <?php echo e($themeColor); ?> 0%, <?php echo adjustColor($themeColor,-15); ?> 100%);
    --primary-light: <?php echo adjustColor($themeColor,20); ?>;
    --primary-dark: <?php echo adjustColor($themeColor,-20); ?>;
    --shadow-primary: 0 4px 20px <?php echo e($themeColor); ?>4D;
}
<?php
function adjustColor($c, $p){$c=str_replace('#','',$c);if(strlen($c)!==6)return $c;$r=hexdec(substr($c,0,2));$g=hexdec(substr($c,2,2));$b=hexdec(substr($c,4,2));$r=max(0,min(255,$r+round(255*$p/100)));$g=max(0,min(255,$g+round(255*$p/100)));$b=max(0,min(255,$b+round(255*$p/100)));return '#'.sprintf('%02x%02x%02x',$r,$g,$b);}
?>
</style>
</head>
