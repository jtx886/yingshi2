                ...
                <div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:20px;">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(100px, 1fr));gap:10px;">
                        <?php foreach ($displayEpisodes as $epItem):
                            ...
                            $link = '?id='.$id.'&type='.$type.'&season='.$season.'&ep='.$epNum.'&dub='.($dub ?: 'original');
                            if ($sourceId) $link .= '&source='.$sourceId;
                        ?>
                        <a href="<?php echo e($link); ?>"
                           style="display:flex;align-items:center;justify-content:center;height:44px;padding:0 12px;border-radius:var(--radius-md);font-size:13px;font-weight:500;transition:var(--transition);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;<?php echo $active ? 'background:var(--primary-gradient);color:white;box-shadow:var(--shadow-primary);' : 'background:var(--bg-dark);border:1px solid var(--border-color);color:var(--text-secondary);'; ?>"
                           title="<?php echo e($epName); ?>"
                           <?php echo $active ? '' : 'onmouseover="this.style.background=\\'var(--bg-card-hover)\\';this.style.color=\\'var(--text-primary)\\'" onmouseout="this.style.background=\\'var(--bg-dark)\\';this.style.color=\\'var(--text-secondary)\\'""; ?>>
